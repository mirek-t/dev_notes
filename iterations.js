...
const sum = people.map(person => person.age).reduce((acc, age) => acc + age)/people.length;
// console.log(sum);
...
const jsLike = people.filter(person => person.hobbies.includes('js'))
.map(person => person.name)
// console.log(jsLike);

const hobbiesList = new Set(people.reduce((acc, person) => [...acc,  ...person.hobbies], [] ))
